import {TokenType as T, Token, createToken, parseTokens} from "./token";
import {processNumberTokens}   from "./number";
import {processUnitTokens}     from "./unit";
import {processReservedTokens} from "./reserved";
import {processEntityTokens}   from "./entity";
...
const NULLORDER: InformalizeSubstage[] = [
  {t: [T.KEYWORD, T.KEYWORD, T.ORDINAL], v: [/SELECT/, /NULL/, /1/],           f: (s, t, i) => createToken(T.KEYWORD, 'NULLS FIRST')},
  {t: [T.KEYWORD, T.KEYWORD, T.TEXT],    v: [/SELECT/, /NULL/, /last|first/i], f: (s, t, i) => createToken(T.KEYWORD, `NULLS ${t[i+2].value.toUpperCase()}`)},
];


/** Rules for processing number-related tokens. */
const NUMBER: InformalizeSubstage[] = [
  {t: [T.CARDINAL, T.ORDINAL], v: [null, null], f: (s, t, i) => createToken(T.CARDINAL, t[i].value/t[i+1].value)},
  {t: [T.CARDINAL, T.UNIT],    v: [null, null], f: (s, t, i) => createToken(T.CARDINAL, t[i].value*t[i+1].value)},
];
...
const VALUE: InformalizeSubstage[] = [
  {t: [T.OPERATOR, T.KEYWORD, T.COLUMN], v: [/ALL/, /TYPE/, null], f: (s, t, i) => createToken(T.COLUMN, `all: ${t[i+2].value}`)},
  {t: [T.OPERATOR, T.KEYWORD, T.COLUMN], v: [/\+/, /TYPE/, null],  f: (s, t, i) => createToken(T.COLUMN, `sum: ${t[i+2].value}`)},
  {t: [T.FUNCTION, T.KEYWORD, T.COLUMN], v: [/avg/, /TYPE/, null], f: (s, t, i) => createToken(T.COLUMN, `avg: ${t[i+2].value}`)},
  {t: [T.COLUMN, T.KEYWORD, T.CARDINAL], v: [null, /PER/, null],   f: (s, t, i) => { s.columnsUsed.push(`"${t[i].value}"`); return createToken(T.EXPRESSION, `("${t[i].value}"*${t[i+2].value/100})`); }},
  {t: [T.COLUMN, T.KEYWORD, T.UNIT],     v: [null, /PER/, null],   f: (s, t, i) => { s.columnsUsed.push(`"${t[i].value}"`); return createToken(T.EXPRESSION, `("${t[i].value}"*${t[i+2].value/100})`); }},
  ...
  {t: [T.NUMBER],                        v: [null],                f: (s, t, i) => createToken(T.VALUE, `${t[i].value}`)},
  {t: [T.TEXT],                          v: [null],                f: (s, t, i) => createToken(T.VALUE, `'${t[i].value}'`)},
  {t: [T.KEYWORD],                       v: [/NULL/],              f: (s, t, i) => createToken(T.VALUE, t[i].value)},
  {t: [T.KEYWORD],                       v: [/TRUE|FALSE/],        f: (s, t, i) => createToken(T.BOOLEAN, t[i].value)},
];
// T.VALUE: t[i].type
...
const EXPRESSION: InformalizeSubstage[] = [
  ...
  {t: [T.EXPRESSION, T.EXPRESSION, T.CLOSE],                             v: [null, null, null],                                  f: (s, t, i) => [createToken(T.EXPRESSION, `${t[i].value}, ${t[i+1].value}`), t[i+2]]},
  {t: [T.FUNCTION],                                                      v: [/pi|random/],                                       f: (s, t, i) => createToken(T.EXPRESSION, `${t[i].value}()`)},
  {t: [T.FUNCTION, T.OPEN, T.EXPRESSION, T.CLOSE],                       v: [null, null, null, null],                            f: (s, t, i) => createToken(T.EXPRESSION, `${t[i].value}(${t[i+2].value})`)},
  {t: [T.FUNCTION, T.EXPRESSION],                                        v: [null, null],                                        f: (s, t, i) => createToken(T.EXPRESSION, `${t[i].value}(${t[i+1].value})`)},
  {t: [T.OPEN, T.EXPRESSION, T.CLOSE],                                   v: [null, null, null],                                  f: (s, t, i) => createToken(T.EXPRESSION, `(${t[i+1].value})`)},
  {t: [T.OPERATOR, T.BINARY, T.EXPRESSION],                              v: [null, /\+|\-/, null],                               f: (s, t, i) => [t[i], createToken(t[i+2].type, `${t[i+1].value}${t[i+2].value}`)]},
  {t: [T.EXPRESSION, T.BINARY, T.EXPRESSION],                            v: [null, /\^/, null],                                  f: (s, t, i) => createToken(t[i].type & t[i+2].type, `power(${t[i].value},  ${t[i+2].value})`)},
  {t: [T.EXPRESSION, T.BINARY, T.EXPRESSION],                            v: [null, /[\*\/%]/, null],                             f: (s, t, i) => createToken(t[i].type & t[i+2].type, `${t[i].value} ${t[i+1].value} ${t[i+2].value}`)},
  {t: [T.EXPRESSION, T.BINARY, T.EXPRESSION],                            v: [null, /[\+\-]/, null],                              f: (s, t, i) => createToken(t[i].type & t[i+2].type, `${t[i].value} ${t[i+1].value} ${t[i+2].value}`)},
  {t: [T.UNARY, T.EXPRESSION],                                           v: [/[^(NOT)]/, null],                                  f: (s, t, i) => createToken(t[i+1].type, `${t[i].value} ${t[i+1].value}`)},
  {t: [T.EXPRESSION, T.UNARY],                                           v: [null, /IS.*/],                                      f: (s, t, i) => createToken(T.BOOLEAN, `${t[i].value} ${t[i+1].value}`)},
  {t: [T.EXPRESSION, T.BINARY, T.EXPRESSION],                            v: [null, /[^\w\s=!<>]+/, null],                        f: (s, t, i) => createToken(T.VALUE, `${t[i].value} ${t[i+1].value} ${t[i+2].value}`)},
  {t: [T.EXPRESSION, T.TERNARY, T.EXPRESSION, T.OPERATOR, T.EXPRESSION], v: [null, null, null, /AND/, null],                     f: (s, t, i) => createToken(T.BOOLEAN, `${t[i].value} ${t[i+1].value} ${t[i+2].value} AND ${t[i+4].value}`)},
  {t: [T.EXPRESSION, T.TERNARY, T.EXPRESSION, T.EXPRESSION],             v: [null, null, null, null],                            f: (s, t, i) => createToken(T.BOOLEAN, `${t[i].value} ${t[i+1].value} ${t[i+2].value} AND ${t[i+3].value}`)},
  {t: [T.EXPRESSION, T.BINARY, T.EXPRESSION, T.OPERATOR, T.EXPRESSION],  v: [null, null, null, /ESCAPE/, null],                  f: (s, t, i) => createToken(T.BOOLEAN, `${t[i].value} ${t[i+1].value} ${t[i+2].value} ESCAPE ${t[i+4].value}`)},
  ...
  {t: [T.KEYWORD, T.VALUE, T.BINARY, T.VALUE, T.OPERATOR, T.VALUE],      v: [null, null, /[^(OR)|(AND)]/, null, /OR|AND/, null], f: (s, t, i) => i+6>=t.length? [t[i], createToken(T.BOOLEAN, `${t[i+1].value} ${t[i+2].value} ${t[i+3].value} AND ${t[i+1].value} ${t[i+2].value} ${t[i+5].value}`)]:t.slice(i, i+6)},
  {t: [T.KEYWORD, T.VALUE, T.OPERATOR, T.VALUE, T.BINARY, T.VALUE],      v: [null, null, /OR|AND/, null, /[^(OR)|(AND)]/, null], f: (s, t, i) => i+6>=t.length? [t[i], createToken(T.BOOLEAN, `${t[i+1].value} ${t[i+4].value} ${t[i+5].value} AND ${t[i+3].value} ${t[i+4].value} ${t[i+4].value}`)]:t.slice(i, i+6)},
  {t: [T.EXPRESSION, T.BINARY, T.EXPRESSION],                            v: [null, /[^(OR)(AND)]/, null],                        f: (s, t, i) => createToken(T.BOOLEAN, `${t[i].value} ${t[i+1].value} ${t[i+2].value}`)},
  {t: [T.UNARY, T.EXPRESSION],                                           v: [null, null],                                        f: (s, t, i) => createToken(T.BOOLEAN, `${t[i].value} ${t[i+1].value}`)},
  {t: [T.VALUE, T.BINARY, T.VALUE],                                      v: [null, /AND/, null],                                 f: (s, t, i) => createToken(T.VALUE, `${t[i].value} + ${t[i+2].value}`)},
  {t: [T.BINARY, T.VALUE],                                               v: [/AND/, null],                                       f: (s, t, i) => { s.columnsUsed.push(t[i+1].value); return t[i+1]; }},
  {t: [T.EXPRESSION, T.BINARY, T.EXPRESSION],                            v: [null, null, null],                                  f: (s, t, i) => createToken(T.BOOLEAN, `${t[i].value} ${t[i+1].value} ${t[i+2].value}`)},
];
 ...
 * @returns true if the tokens match the pattern, false otherwise
 */
function matchValueInformalize(tokens: Token[], i: number, val: any[]) {
  for (var j=0, J=val.length; j<J; i++, j++)
    if (val[j]!=null && !val[j].test(tokens[i].value)) return false;
  return true;
}
...
function runInformalizeSubstage(substage: InformalizeSubstage, state: InformalizeState, tokens: Token[], repeat=false) {
  ...
  do {
    var tokens = a; a = [];
    for (var i=0, I=tokens.length; i<I; i++) {
      if (!matchTokenTypeInformalize(tokens, i, substage.t) || !matchValueInformalize(tokens, i, substage.v)) {
        a.push(tokens[i]); continue;
      }
      var ans = substage.f(state, tokens, i);
...
function runInformalizeStage(stage: InformalizeSubstage[], state: InformalizeState, tokens: Token[], rsubstage=false, rstage=false) {
  var a = tokens;
  do {
    var plen = tokens.length;
    for (var sub of stage)
 ...
 */
function processColumnsInformalize(state: InformalizeState, opt: any={}): string[] {
  var cols = new Set<string>(state.columns);
  if (cols.has('"*"')) cols.add('*');
  cols.delete('"*"');
  if (cols.size===0 || !cols.has('*')) {
    for (var ord of state.orderBy)
      cols.add(ord.replace(/ (ASC|DESC)$/, ''));
    for (var col of state.groupBy.length? [] : state.columnsUsed)
      cols.add(col);
  }
  var colt = new Set<string>(state.groupBy);
  if (!cols.has('*')) {
    for (var hnt of state.hints)
 ...
 */
function processTokensInformalize(tokens: Token[], opt: any={}) {
  var s: InformalizeState = {
    columns: [],
    from:    [],
    groupBy: [],
    orderBy: [],
  ...
  for (var t of tokens)
    if (t.hint!=null) s.hints.add(t.hint);
  ...
  var a = `SELECT ${s.columns.join(', ')} FROM ${s.from.join(', ')}`;
 ...
 */
export async function convertToInformalSQL(txt: string, fn: Function, ths: any=null, opt: any={}) {
  var tokens = parseTokens(txt);
  ...
  tokens = tokens.filter((v) => v.type!==T.TEXT || !/[~!@#$:,\?\.\|\/\\]/.test(v.value));
