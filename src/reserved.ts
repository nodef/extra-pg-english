import {PorterStemmer, LancasterStemmer} from "natural";
...
function stemReservedText(txt: string): string {
  if (!LANCASTER.test(txt)) return PorterStemmer.stem(txt);
  return LancasterStemmer.stem(txt);
}
...
function findLastTokenOfType(tokens: Token[], i: number, type: T) {
  var a = -1;
  for (var I=tokens.length; i<I; a=i++)
...
function processReservedTexts(txts: string[]): Token | null {
  var a: Token | null = null;
  for (var i=0, I=RESERVED_STEPS.length; i<I && a==null; i++)
...
export function processReservedTokens(tokens: Token[]): Token[] {
  ...
  for (var i=0, I=tokens.length; i<I; i++) {
    var J = findLastTokenOfType(tokens, i, T.TEXT);
    if (J<0) { a.push(tokens[i]); continue; }
    var words = tokens.slice(i, J+1).map((v: Token) => v.value.toLowerCase());
    for (var j=J; j>=i; j--) {
      var rsv = processReservedTexts(words);
