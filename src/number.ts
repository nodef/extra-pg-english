...
function numberStateValue(s: number[], pre=NaN) {
  ...
  for (var i=s.length; i>3; i-=3) numberStateFuse(s, i);
...
export function processNumberTokens(tokens: Token[]): Token[] {
  var s = [], pre = NaN, val = false;
  var brk: Token | null = null;
  var a: Token[] = [];
  for (var t of tokens) {
    var txt = (t.type & 0xF0)===T.TEXT? t.value.replace(/[\s,]/g, '').toLowerCase() : null;
    if (val && (!txt || brk)) {
